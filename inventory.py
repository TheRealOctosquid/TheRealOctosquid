...
# Creating a heading , and body for the reciept , defining print
def print():
    tott = float(toText.get())
    top = Toplevel()
...
# this is where  item boxes are created to store values
...
def show():
    ...
    if (var1.get()):
        ...
        qty = int(e6.get())
        tot = int(price * qty)
        tempList = [['Sticker', e1.get(), e6.get(), tot]]
    ...
    if (var2.get()):
        ...
        qty = int(e7.get())
        tot = int(price * qty)
        tempList = [['Hoodie', e2.get(), e7.get(),tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
    ...
    if (var3.get()):
        ...
        qty = int(e8.get())
        tot = int(price * qty)
        tempList = [['Shoe', e3.get(), e8.get(),tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
    ...
    if (var4.get()):
        ...
        qty = int(e9.get())
        tot = int(price * qty)
        tempList = [['T-shirt', e4.get(), e9.get(),tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
    ...
    if (var5.get()):
        ...
        qty = int(e10.get())
        tot = int(price * qty)
        tempList = [['Hat', e5.get(), e10.get(),tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
...
#Creating the title and size of program interface

root = Tk()
root.title("Reciept Print Inventory System")
...
Label(root, text="Reciept Print inventory System", font="arial 22 bold", bg="white").place(x=5, y=10)


var1 = IntVar()
Checkbutton(root, text="Sticker", variable=var1).place(x=10, y=50)

var2 = IntVar()
Checkbutton(root, text="Hoodie", variable=var2).place(x=10, y=80)

var3 = IntVar()
Checkbutton(root, text="Shoe", variable=var3).place(x=10, y=110)

var4 = IntVar()
Checkbutton(root, text="T-Shirt", variable=var4).place(x=10, y=140)

var5 = IntVar()
Checkbutton(root, text="Hat", variable=var5).place(x=10, y=170)
...
tot = Label(root, text="", font="arial 22 bold", textvariable=toText)
tot.place(x=650, y=10)


# Creating the buttons for Add and print
